...
ssize_t read_from_memory(task_t task, void *buffer, size_t nbyte) {
  ...
  kern_return_t kr = mach_vm_read_overwrite(
      task, current_address, nbyte, (mach_vm_address_t)buffer, &outsize);
  if (kr == KERN_SUCCESS) {
...
uint64_t getImageSize64(task_t task, struct mach_header_64 *header) {
  ...
  for (int i = 0; i < header->ncmds; i++) {
    ...
    if (lc.cmd == LC_SEGMENT_64) {
      ...
      lseek_from_memory(task, lc.cmdsize - sizeof(struct load_command),
                        SEEK_CUR);
    }
  }
  return image_size;
...
unsigned long long GetModuleSize(task_t task, void *lpAddress,
  ...
  if (magic == MH_MAGIC_64) {
    ...
    struct fat_arch *archs = (struct fat_arch *)malloc(fatHeader.nfat_arch *
                                                       sizeof(struct fat_arch));
    read_from_memory(task, archs,
