...
HANDLE OpenProcess(DWORD pid)
{
    ...
    int handle = SearchHandleList(htProcesHandle,
                                  (HANDLESEARCHCALLBACK)SearchHandleListProcessCallback, &pid);
    if (handle)
    {
...
int WriteProcessMemory(HANDLE hProcess, void *lpAddress, void *buffer, int size)
{
    if (GetHandleType(hProcess) == htProcesHandle)
    {
        ...
        kern_return_t kr =
            mach_vm_write(p->task, (mach_vm_address_t)lpAddress, (vm_offset_t)buffer, size);
        if (kr != KERN_SUCCESS)
        {
...
int ReadProcessMemory(HANDLE hProcess, void *lpAddress, void *buffer, int size)
{
    if (GetHandleType(hProcess) == htProcesHandle)
    {
        ...
        kern_return_t kr = mach_vm_read_overwrite(p->task, (mach_vm_address_t)lpAddress, size,
...
HANDLE CreateToolhelp32Snapshot(DWORD dwFlags, DWORD th32ProcessID)
{
    if (dwFlags & TH32CS_SNAPPROCESS)
    {
        ...
        do
        {
            ...
            if (err == 0)
            {
                err = sysctl((int *)name, (sizeof(name) / sizeof(*name)) - 1, result, &length, NULL,
                             0);
                if (err == -1)
                {
                    err = errno;
        ...
        for (int i = 0; i < procCount; i++)
        {
            ...
            pl->processList[pl->processCount].ProcessName = strdup(result[i].kp_proc.p_comm);
            ...
            if (pl->processCount >= max)
            {
                max = max * 2;
                pl->processList =
                    (PProcessListEntry)realloc(pl->processList, max * sizeof(ProcessListEntry));
            }
        }
        return CreateHandleFromPointer(pl, htTHSProcess);
    }
    else if (((dwFlags & TH32CS_SNAPMODULE) || (dwFlags & TH32CS_SNAPFIRSTMODULE)))
    {
        ...
        if (task_info(p->task, TASK_DYLD_INFO, (task_info_t)&dyld_info, &count) == KERN_SUCCESS)
        {
            mach_msg_type_number_t size = sizeof(dyld_all_image_infos);

            uint8_t *data = (uint8_t *)malloc(size);
            ReadProcessMemory(pHandle, (void *)dyld_info.all_image_info_addr, data, size);
            struct dyld_all_image_infos *infos = (struct dyld_all_image_infos *)data;

            mach_msg_type_number_t size2 = sizeof(dyld_image_info) * infos->infoArrayCount;
            ...
            for (int i = 0; i < infos->infoArrayCount; i++)
            {
                ...
                ReadProcessMemory(pHandle, (void *)info[i].imageFilePath, fpath_addr, size3);
                if (fpath_addr)
                {
                    ...
                    if (strlen((const char *)fpath_addr) == 0)
                    {
                        ...
                        int ret = proc_regionfilename(p->pid, (uint64_t)info[i].imageLoadAddress,
                    ...
                    // mle->fileOffset = 0;
                    mle->moduleSize = GetModuleSize(p->task, (void *)mle->baseAddress, 0, 0);
                    mle->part = 0;
                    mle->is64bit = 1;
                    ml->moduleCount++;
                    if (ml->moduleCount >= max)
                    {
                        ...
                        max = max * 2;
                        ml->moduleList = (PModuleListEntry)realloc(ml->moduleList,
                                                                   max * sizeof(ModuleListEntry));
                    }

                    if (dwFlags & TH32CS_SNAPFIRSTMODULE) break;
                }
            }
            qsort(ml->moduleList, ml->moduleCount, sizeof(ModuleListEntry), compare_module);
        }
    ...
    if (GetHandleType(hSnapshot) == htTHSProcess)
    {
        ...
        if (pl->processListIterator < pl->processCount)
        {
            ...
            processentry->ProcessName =
                pl->processList[pl->processListIterator].ProcessName;  // no need to copy
...
void CloseHandle(HANDLE h)
{
    ...
    if (ht == htTHSModule)
    {
        ...
        if (ml->ReferenceCount <= 0)
        {
            ...
            for (i = 0; i < ml->moduleCount; i++) free(ml->moduleList[i].moduleName);
    ...
    else if (ht == htTHSProcess)
    {
        ...
        {
            // free all the processnames in the list
            for (i = 0; i < pl->processCount; i++) free(pl->processList[i].ProcessName);
...
void AddToRegionList(uint64_t base, uint64_t size, uint32_t type, uint32_t protection,
                     RegionInfo **list, int *pos, int *max)
 ...
 */
{
    ...
    if (GetHandleType(hProcess) == htProcesHandle)
    {
        ...
        while (true)
        {
            ...
            if (vm_region_recurse_64(p->task, &address, &size, &depth, (vm_region_info_t)&info,
...
int VirtualQueryEx(HANDLE hProcess, void *lpAddress, PRegionInfo rinfo, char *mapsline)
{
    ...
    if (GetHandleType(hProcess) == htProcesHandle)
    {
        ...
        while (true)
        {
            if (vm_region_recurse_64(p->task, (vm_address_t *)&lpAddress, &size, &depth,
