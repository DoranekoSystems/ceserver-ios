...
void InitializeProcessThreadlist(PProcessData p)
{
    ...
    {
        p->threadlist = (ThreadData *)malloc(sizeof(ThreadData) * 64);  // preallocate room for 64
        p->threadlistmax = 64;
    }
...
{
    if (p->threadlist == NULL) InitializeProcessThreadlist(p);

    if (p->threadlistpos == p->threadlistmax)
    {
        // realloc
        p->threadlist =
            (ThreadData *)realloc(p->threadlist, sizeof(ThreadData) * p->threadlistmax * 2);
...
int RemoveThreadFromProcess(PProcessData p, int tid)
{
    ...
    for (i = 0; i < p->threadlistpos; i++)
        if (p->threadlist[i].tid == tid)
        {
            // found it
            int j;
            for (j = i; j < p->threadlistpos - 1; j++) p->threadlist[j] = p->threadlist[j + 1];
...
PThreadData GetThreadData(PProcessData p, int tid)
{
    int i;
    for (i = 0; i < p->threadlistpos; i++)
        if (p->threadlist[i].tid == tid) return &p->threadlist[i];
