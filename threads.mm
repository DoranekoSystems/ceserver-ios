...
void InitializeProcessThreadlist(PProcessData p) {
  ...
  {
    p->threadlist = (ThreadData *)malloc(sizeof(ThreadData) *
                                         64); // preallocate room for 64
    p->threadlistmax = 64;
  }
...
void AddThreadToProcess(PProcessData p, PThreadData threaddata) {
  ...
  if (p->threadlistpos == p->threadlistmax) {
    // realloc
    p->threadlist = (ThreadData *)realloc(
        p->threadlist, sizeof(ThreadData) * p->threadlistmax * 2);
    if (p->threadlist == NULL) {
