...
LLDBAutomation::LLDBAutomation(const std::string &server_ip, int server_port) : wpinfo(4)
{
...
std::string LLDBAutomation::send_message(const std::string &message, bool recvflag)
{
...
std::string LLDBAutomation::cont()
{
...
bool LLDBAutomation::set_watchpoint(unsigned long long address, int size, char _type)
{
    std::string command = "";
    if (_type == 'x')
    {
    ...
    std::string result = send_message(command + "," + ss.str() + "," + std::to_string(size));
    if (result == "OK")
    {
        return true;
    }
...
bool LLDBAutomation::remove_watchpoint(unsigned long long address, int size, char _type)
{
    std::string command = "";
    if (_type == 'x')
    {
...
std::map<std::string, std::string> LLDBAutomation::parse_result(const std::string &result)
{
