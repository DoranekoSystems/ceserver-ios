...
std::map<int, std::string> command_map = {{0, "CMD_GETVERSION"},
    ...
    switch (command)
    {
        ...
        case CMD_VIRTUALQUERYEX:
        {
            ...
            if (r > 0)
            {
                ...
                if (command == CMD_VIRTUALQUERYEX)
                    o.result =
                        VirtualQueryEx(c.handle, (void *)(uintptr_t)c.baseaddress, &rinfo, NULL);
        ...
        case CMD_GETSYMBOLLISTFROMFILE:
        {
            ...
            if (recvall(currentsocket, symbolpath, input.symbolpathsize, MSG_WAITALL) > 0)
            {
                ...
                if (input.fileoffset) debug_log("symbolpath=%s\n", symbolpath);
                ...
                {
                    if (input.fileoffset) debug_log("Sending 8 bytes (fail)\n");
        ...
        case CMD_MODULE32NEXT:
        {
            ...
            if (recvall(currentsocket, &toolhelpsnapshot, sizeof(toolhelpsnapshot), MSG_WAITALL) >
                0)
            {
                bool result;
                ModuleListEntry me;
        ...
        case CMD_READPROCESSMEMORY:
        {
            ...
            if (r > 0)
            {
                ...
                o->read =
                    ReadProcessMemory((int)c.handle, (void *)(uintptr_t)c.address, &o[1], c.size);
                if (c.compress)
                {
// compress the output
#define COMPRESS_BLOCKSIZE (64 * 1024)
                    ...
                    unsigned char **compressedBlocks = (unsigned char **)malloc(
                        maxBlocks *
                        sizeof(unsigned char *));  // send in blocks of 64kb and reallocate the
            ...
            if (r > 0)
            {
                ...
                if (c.size)
                {
                    ...
                    if (r > 0)
                    {
                        ...
                        o.written =
                            WriteProcessMemory(c.handle, (void *)(uintptr_t)c.address, buf, c.size);
        ...
        case CMD_CREATETOOLHELP32SNAPSHOTEX:
        {
            ...
            if (recvall(currentsocket, &params, sizeof(CeCreateToolhelp32Snapshot), MSG_WAITALL) >
                0)
            {
                ...
                if ((params.dwFlags & TH32CS_SNAPTHREAD) == TH32CS_SNAPTHREAD)
                {
                    ...
                    if (r)
                    {
                        ...
                        sendall(currentsocket, &tl->threadList[0], tl->threadCount * sizeof(int),
                                0);
